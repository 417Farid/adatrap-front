...
const Bars = () => {
  const { data } = useFetch("http://127.0.0.1:8000/frecuencia-dia");
  ...
  const dias = data?.map((item) => getNombreDias(item.dia_semana));
  const promedios = data?.map((item) => item.promedio_diferencia_dia_semana);
